...
def main():
    ...
    tab_selection = st.selectbox("What do you want to migrate?", ["Migrate Configurations", "Migrate Storage"])

    if source_project_host and source_api_token and destination_project_host and destination_api_token:
        if tab_selection == "Migrate Configurations":
        ...
        elif tab_selection == "Migrate Storage":
            # Migrate Storage Object Definitions in the main area
            st.header("Migrate Storage Object Definitions")

            # Fetch buckets and tables details
            buckets_to_migrate, tables_to_migrate = export_storage_metadata(source_project_host, source_api_token)
            
            # Destination project has native data types feature
            native_data_types = st.checkbox("Destination project should use Native Data Types", value=False)
            
            # Migrate only selected buckets or tables
            only_selected_objects = st.checkbox("Migrate only selected buckets", value=False)
            
            if only_selected_objects:
                bucket_ids_to_migrate_selection = st.multiselect("Bucket IDs", [buckets_to_migrate[i]['id'] for i in range(len(buckets_to_migrate))])
            else:
                bucket_ids_to_migrate_selection = None

            # Migrate
            if st.button("Migrate Storage Object Definitions"):
                # Execute migrate storage object definitions script
                if not native_data_types:
                    migrate_buckets(destination_project_host, destination_api_token, buckets_to_migrate, bucket_ids_to_migrate_selection)
                    migrate_tables(destination_project_host, destination_api_token, tables_to_migrate, bucket_ids_to_migrate_selection)
                    st.write("Migration completed.")
                else: 
                    migrate_buckets(destination_project_host, destination_api_token, buckets_to_migrate, bucket_ids_to_migrate_selection)
                    migrate_tables_native_types(destination_project_host, destination_api_token, tables_to_migrate, bucket_ids_to_migrate_selection)
                    st.write("Migration completed.")
    else:
